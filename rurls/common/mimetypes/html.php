	...
	function get_fetched_data( $html ) {
		...
		$data = array(
			...
			'image'       => array(
				...
				'src' => '',
				// the generated image id if successfully sideloaded
				'id'  => FALSE,
			),
		);
	...
	function template_html_row( $data ) {
		...
		<div class="rurls-html-row">
			...
			<?php if ( $data['image']['id'] ) { ?>
				<span class="rurls-html-image"><?php print wp_get_attachment_image( $data['image']['id'], $this->settings['image_size'] ); ?></span>
			<?php } ?>
